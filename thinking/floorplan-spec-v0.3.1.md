# Floor Plan & Space Management — Spec v0.3.1

**Supersedes:** `floorplan-spec-v0.3.md`

**Status:** This spec reflects the floor plan system as built and extends it
with the storage plan model. Sections marked **[NEW v0.3.1]** are additions or
significant expansions over v0.3. Everything else is carried forward unchanged.
...
**[NEW v0.3.1] Shelf bay** — a physical shelving unit occupying a fixed floor
footprint. A bay has N vertical levels, each independently rentable. The bay
footprint appears once per level layer in the DXF; each occurrence produces a
distinct rentable Space + Resource.

**[NEW v0.3.1] Shelf level** — one horizontal shelf within a bay. Identified by
the DXF layer it appears on (`shelf_l1` = lowest, `shelf_l2` = one above, etc.).
The number of levels in a bay equals the number of shelf layers that contain its
block insert.
...
| `storage` | Pallet and tool cart block inserts | Block name + position |
...
| `shelf_l4` | Shelf bay blocks, level 4 | Block name + position |
| `shelf_l5` | Shelf bay blocks, level 5 | Block name + position |
...
| `storage_label` | **[NEW v0.3.1]** TEXT labels for bay identifier codes | — |
| `fp_marker` | Provenance marker (see below) | — |

Additional shelf levels beyond 5 follow the same pattern (`shelf_l6`, etc.).
The pipeline treats any layer matching `shelf_l\d+` as a shelf level layer.
...
### [NEW v0.3.1] Storage Blocks

| Block name | Description | Layer |
|---|---|---|
| `pallet-std` | Standard pallet bay (48"x40") | `storage` |
| `sb-std` | Standard shelf bay (standard depth) | `shelf_l*` |
| `sb-wide` | Wide shelf bay | `shelf_l*` |
| `tc-std` | Tool cart space | `storage` |

Block dimensions are defined in the DXF library maintained by the facility's
CAD operator. The pipeline does not enforce specific dimensions — it reads
whatever blocks are present and matches them by name prefix.
...
### [NEW v0.3.1] Storage Bay Numbering

Bay identity follows the same pattern as studio numbering but uses the
`storage_label` layer.

The pipeline:
1. Reads existing bay codes from `storage_label` (TEXT entities, e.g. `A1`, `B3`)
2. Preserves existing codes matched by proximity to bay centroid (same tolerance
   as studio labels)
3. Assigns new sequential codes to unlabeled bays: `A1`, `A2`… `A26`, `B1`…
   (column letter + row number, west→east, north→south)
4. Writes new codes back to `storage_label` in the labeled DXF output

Bay codes are building-scoped, not floor-scoped (to avoid ambiguity in multi-floor
storage areas). The first letter indicates the zone/row, the number the bay within
it.

**Shelf level identity** is derived from the DXF layer, not from a label. A block
insert for bay `A1` on layer `shelf_l2` produces `shelf-A1-l2`. No additional label
is required.

**`externalId` format for storage spaces:**

| Type | Format | Example |
|---|---|---|
| Pallet | `pallet-{code}` | `pallet-A1` |
| Shelf level | `shelf-{bay_code}-l{level}` | `shelf-B3-l2` |
| Tool cart | `toolcart-{code}` | `toolcart-C2` |
...
6. For each INSERT on layer `storage`:
   - Look up or assign bay code from `storage_label`
   - Determine subtype from block name prefix (`pallet-*` → pallet, `tc-*` → tool_cart)
   - Emit `<polygon data-space-id="pallet-A1" data-type="pallet" data-block="pallet-std" data-bay="A1" .../>`
7. **[NEW v0.3.1]** For each shelf level layer matching `shelf_l\d+`:
   - Extract level number N from layer name
   - For each INSERT on this layer:
     - Match to bay code by centroid proximity to `storage_label` TEXT
       (a shelf insert shares the same XY footprint as its floor-level bay)
     - Emit `<polygon data-space-id="shelf-A1-l2" data-type="shelf"
         data-block="sb-std" data-bay="A1" data-level="2" data-layer="shelf_l2" .../>`
   - Each shelf level layer produces a separate SVG `<g data-shelf-layer="shelf_l2">` group
8. Render common area elements similarly
9. Output SVG to `public/floorplans/`

### [NEW v0.3.1] Shelf Layer Grouping in SVG

All polygons for a given shelf level are wrapped in a group:

```svg
<g data-shelf-layer="shelf_l1" class="shelf-layer" data-level="1">
  <polygon data-space-id="shelf-A1-l1" data-bay="A1" data-level="1" ... />
  <polygon data-space-id="shelf-B1-l1" data-bay="B1" data-level="1" ... />
</g>
<g data-shelf-layer="shelf_l2" class="shelf-layer" data-level="2">
  <polygon data-space-id="shelf-A1-l2" data-bay="A1" data-level="2" ... />
  ...
</g>
```

This grouping allows the browser to show/hide levels by toggling `display` on
the group. Polygons for different levels of the same bay are co-located in XY —
...
- **[NEW v0.3.1]** Bay codes for newly coded storage bays on `storage_label` layer
...
5. **User downloads labeled DXF** — must save for future revisions (contains
   provenance marker and bay codes)
...
- **[NEW v0.3.1]** For shelf spaces, also stores `data-bay` and `data-level`
  attributes (see schema below)
- Generates display names from externalId (replaces underscores/colons with spaces)
...
model Space {
  ...
  // [NEW v0.3.1] Shelf-specific fields — null for non-shelf spaces
  bayCode     String?              // e.g. "A1" — the parent physical bay
  shelfLevel  Int?                 // 1 = lowest physical level
...
**Changes from v0.3:**
- `Space.bayCode` — identifies which physical bay a shelf space belongs to
  (enables "show all levels of bay A1" queries)
- `Space.shelfLevel` — the vertical position; 1 = floor level (lowest)
...
## [NEW v0.3.1] Resource Model for Storage

### Pallet and Tool Cart

1:1 mapping — one Space → one Resource of type `pallet` or `tool_cart`.
No parent/child relationship needed.

### Shelf Bays — Resource Hierarchy

A bay is modelled as a parent `Resource` (typeTag `storage_bay`, not directly
rentable) with child `Resource` records per level (typeTag `shelf`, leasable).

```
Resource (typeTag: storage_bay, name: "Bay A1", leasable: false)
  ├── Resource (typeTag: shelf, name: "Bay A1 — Level 1", leasable: true)
  │     └── Space (externalId: shelf-A1-l1)
  ├── Resource (typeTag: shelf, name: "Bay A1 — Level 2", leasable: true)
  │     └── Space (externalId: shelf-A1-l2)
  └── Resource (typeTag: shelf, name: "Bay A1 — Level 3", leasable: true)
        └── Space (externalId: shelf-A1-l3)
```

This mirrors the studio/studio_unit pattern. The bay resource provides grouping
and display context; only level resources appear in rental workflows.

### Creating Bay Resources from the Floor Plan

After upload, the admin storage management page lists all bay codes found in the
SVG (from `Space.bayCode`). For each uncreated bay, staff can:
1. Click "Create bay" — creates the parent Resource and N child Resources
   (one per level detected in the SVG)
2. Names follow the convention: "Bay {code} — Level {N}"
3. Resource is immediately leasable

This mirrors the "Create studio from floor plan" workflow in the studios page.

---

## [NEW v0.3.1] Storage Management Admin Page

`/admin/storage`

Analogous to `/admin/studios`. Displays the storage floor plan alongside a
management table.

### Floor Plan Panel

- Storage floor plan always visible (same viewer component)
- **Level selector** — tabs or radio buttons for Level 1 / Level 2 / Level 3 / etc.
  - Switches the visible SVG `<g>` group by toggling `shelf-layer` group display
  - All non-shelf spaces (pallets, tool carts) always visible
- Click on a space highlights the corresponding row in the management table
- Colour states:
  - Vacant shelf level — green
  - Occupied shelf level — red
  - Selected level — amber
  - Pallet/tool cart (vacant) — green
  - Pallet/tool cart (occupied) — red

### Management Table

Columns: Bay code | Level | Dimensions | Tenant | Rental start | Monthly rate | Actions

Groups rows by bay (collapsible). Shows all levels for each bay.

**Actions per row:**
- Vacant: "Create rental request" (admin-initiated, bypasses request queue)
- Occupied: View tenant, "End rental"
- Bay header: "View bay" → bay detail panel

### Bay Detail Panel

Shows all levels, current occupancy, rental history, and the bay's physical
location highlighted on the floor plan.
...
- `data-occupant` — current tenant name (for occupied studios/storage)
- `data-level` — already present for shelf spaces (from pipeline)
- `data-bay` — already present for shelf spaces (from pipeline)
...
| Vacant | `#d4edda` (green) |
| Occupied | `#f8d7da` (red) |
| My studio / my storage | `#cce5ff` (blue) |
| Selected | `#fff3cd` (amber) |
| Common area | `#f5f5f5` (neutral) |
| Not synced | Grey (no resource link) |

### [NEW v0.3.1] Shelf Level Toggles

Shelf levels render as distinct SVG `<g data-shelf-layer="shelf_l*">` groups.
A level selector control (tabs above the floor plan) shows/hides levels by
toggling CSS `display` on the corresponding group. Only one level is visible at
a time. Non-shelf storage (pallets, tool carts) are always visible regardless
of level selection.

Default view: Level 1 (lowest) visible on load.

### Member Portal Floor Plan

- Read-only occupancy view at `/portal/map`
- Hover shows occupant names for all rented spaces
- Level selector visible for storage areas
...
- **[NEW v0.3.1] Create storage bay from floor plan:** Select bay code from
  detected list → system creates parent bay Resource + one child Resource per
  detected level → immediately available for rental
...
| Member assigned to storage | Application DB | Admin UI (storage management page) |
...
| Shelf level added to a bay | DXF | Add INSERT on new `shelf_l*` layer → upload → creates new Space + child Resource |
| Shelf bay added | DXF | Add INSERTs on all relevant `shelf_l*` layers, add label on `storage_label` → upload |
...
- Weight or load capacity tracking for shelf levels
...
- [ ] **[NEW v0.3.1]** Maximum number of shelf levels in use? (determines how many
  `shelf_l*` layers the pipeline needs to handle; currently supports up to 5,
  extending further is trivial)
- [ ] **[NEW v0.3.1]** Are all bays in a given storage area the same depth/width,
  or do bay block variants need to carry dimension metadata?
- [ ] **[NEW v0.3.1]** Pricing model for shelf storage: per-level flat rate, or
  tiered by level height (lower levels more accessible → higher rate)?
- [ ] **[NEW v0.3.1]** Should the member portal map show storage occupancy at all,
  or only staff-visible?

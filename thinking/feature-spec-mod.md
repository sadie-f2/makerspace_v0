...
- Eventbrite integration for class listings    ----- this may written ahead of the larger framework, and then integrated in when that framework is built
...
| Hosting | ~$1,000/year |
| Brivo → Auth0 integration | ~$1,500/year | ---- or write brivo access against the brivo API
...
- Integrated directly with Auth0 (no custom API code required) ---- sub-task, evaluate writing against API vs 
...
- **Not used** for shop or tool physical access — that is trust-enforced
...
/*  correction to this section
    7x24 members may rent studios
    all others may only rent storage space

    we should be able to add membership levels easily & define
    what new levels can do
*/	
...
More exactly how it works now, a day pass user is given one of 12 fobs that gains building access,

those fobs could be tied to the member's known permissions for the period it's active
...
- Proration handled by Stripe ---- not sure if this is stripe, and proration only happens when someone begins, ending an account is always on a month end
...
### Member Portal
- Member can view their certifications   ---- thgere is not compelling privacy reason members should not be allowed to see others' certs
...
- A shop may have 1–20 tools    ----- "many", not 20 .. 20 is probably more than we actually need, but there's no reason for a limit
...
### Member Workflows
- Browse availability calendar across all resource types    ---- we've built this in the booking-app
...
- Display upcoming classes from Eventbrite API in the member portal    ---- EB may be done ahgead and move into phase 1 scope

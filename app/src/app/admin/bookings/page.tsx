...
export default async function AdminBookingsPage({
  ...
  const [reservations, systemConfig] = await Promise.all([
  ...
  async function adminCancelBooking(formData: FormData) {
    "use server";
    await requireUnfrozen("/admin/bookings");
    const id = formData.get("id") as string;
    const session = await auth();
    if (!session?.user.id) redirect("/admin/bookings");

    const reservation = await prisma.reservation.findUnique({ where: { id } });
    if (!reservation) redirect("/admin/bookings");
    ...
    redirect(`/admin/bookings?date=${formData.get("date") ?? todayStr()}`);
  }
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link
          href={`/admin/bookings?date=${prevDate}`}
        ...
        <Link
          href={`/admin/bookings?date=${nextDate}`}
          className="text-sm text-gray-500 hover:text-gray-800 px-2 py-1 rounded border hover:border-gray-400"
        >
          Next →
        </Link>
